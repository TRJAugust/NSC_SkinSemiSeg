...
from Models.Transformer.SwinUnetCCT8 import SwinUnet
import wandb
...
def main(config):
    ...
    train_data, val_data, test_data = get_dataset(config, 
    ...
    l_train_loader = torch.utils.data.DataLoader(train_data['l_dataset'],
                                                batch_size=config.train.l_batchsize,
                                                shuffle=True,
                                                num_workers=config.train.num_workers,
                                                pin_memory=True,
                                                drop_last=False)
    u_train_loader = torch.utils.data.DataLoader(train_data['u_dataset'],
    ...
    print(len(train_data['l_dataset']), len(train_data['u_dataset']))
...
def train_val(config, model, train_loader, val_loader, criterion):
    ...
    for epoch in range(epochs):
        ...
        num_train = 0
        iter = 0
        source_dataset = zip(train_loader['l_loader'], train_loader['u_loader'])
        for idx, (batch, batch_w_s) in enumerate(source_dataset):
            ...
            output, _, _, _, _, _, _, _ = model(img)
            ...
            #======================================================================================================
            # outputs for model
            main_seg, aux1_seg, aux2_seg, aux3_seg, aux4_seg, aux5_seg, aux6_seg, aux7_seg = model(weak_batch)
            ...
            aux4_seg = torch.sigmoid(aux4_seg)
            aux5_seg = torch.sigmoid(aux5_seg)
            aux6_seg = torch.sigmoid(aux6_seg)
            aux7_seg = torch.sigmoid(aux7_seg)
            ...
            for function in criterion:
                ...
                unsup_losses.append(function(main_seg, aux4_seg))
                unsup_losses.append(function(main_seg, aux5_seg))
                unsup_losses.append(function(main_seg, aux6_seg))
                unsup_losses.append(function(main_seg, aux7_seg))
                losses.append(sum(unsup_losses) / 7)
            #======================================================================================================
            ...
            if config.log == True:    
                wandb.log({'Train/loss': loss.item(), 
                       'Train/IOU':iou_train,
                       'Dice_s_loss': round(losses[1].item(), 5),
                       'Dice_u_loss': round(losses[3].item(), 5),
                       'BCE_s_loss': round(losses[0].item(), 5),
                       'BCE_u_loss': round(losses[2].item(), 5)})
        ...
        print('Epoch {}, Validation || sum_loss: {}, Dice score: {}, IOU: {}'.
                format(epoch, round(loss_val_sum.item(),5), 
                round(dice_val_sum,4), round(iou_val_sum,4)))
...
if __name__=='__main__':
    ...
    exp_dir = 'results/{}_{}'.format(args.exp, now.strftime("%Y%m%d_%H%M"))
    ...
    if config.log == True:
        ...
        run = wandb.init(
            ...
            name='{}-{}'.format(args.exp, config['data']['supervised_ratio']),
            config={
                "seed": args.seed,
                "thresh": config.semi.conf_thresh,
            },
        )
