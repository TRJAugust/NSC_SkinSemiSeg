...
def train_val(config, model, train_loader, val_loader, criterion):
    ...
    for epoch in range(epochs):
        ...
        for idx, (batch, 
            ...
            dice_s_loss = losses[1].item()
            bce_s_loss = losses[0].item()
            dice_u_loss = losses_s1[1].item() * 0.25 + losses_s2[1].item() * 0.25 + losses_fp[1].item() * 0.5
            bce_u_loss = losses_s1[0].item() * 0.25 + losses_s2[0].item() * 0.25 + losses_fp[0].item() * 0.5
