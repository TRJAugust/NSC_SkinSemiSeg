...
from Models.Transformer.Allspark import SwinUnet
...
def train_val(config, model, train_loader, val_loader, criterion):
    ...
    for epoch in range(epochs):
        ...
        model.swin_unet.decoder.set_SMem_status(epoch=epoch, isVal=False)
        for idx, (batch, batch_w_s) in enumerate(source_dataset):
            img = batch['image'].cuda().float()
            label = batch['label'].cuda().float()
            img_u_s = batch_w_s['img_s'].cuda().float()
            with torch.no_grad():
                model.eval()
                pred_u_pseudo = model(img_u_s).detach()
                pred_u_pseudo = torch.sigmoid(pred_u_pseudo)
                model.swin_unet.decoder.set_pseudo_prob_map(pred_u_pseudo)
                pseudo_label = torch.round(pred_u_pseudo)
                model.swin_unet.decoder.set_pseudo_label(pseudo_label)
            
            sup_batch_len = img.shape[0]
            unsup_batch_len = img_u_s.shape[0]
            
            output = model(torch.cat((img, img_u_s)))
            output = torch.sigmoid(output)
            pred_x, pred_u = output.split([sup_batch_len, unsup_batch_len])
            
            # calculate loss
            assert (pred_x.shape == label.shape)
            losses_lb = []
            for function in criterion:
                losses_lb.append(function(pred_x, label))
            
            # calculate loss
            assert (pred_u.shape == pseudo_label.shape) 
            losses_ulb = []
            for function in criterion:
                losses_ulb.append(function(pred_u, pseudo_label))
            
            #======================================================================================================
            consistency_weight = get_current_consistency_weight(iter // 150)
            sup_loss = (losses_lb[0] + losses_lb[1]) / 2
            unsup_loss = (losses_ulb[0] + losses_ulb[1]) / 2
            ...
            if config.log == True:    
                wandb.log({'Train/loss': loss.item(), 
                       'Train/IOU':iou_train,
                       'Dice_s_loss': round(losses_lb[1].item(), 5),
                       'Dice_u_loss': round(losses_ulb[1].item(), 5),
                       'BCE_s_loss': round(losses_lb[0].item(), 5),
                       'BCE_u_loss': round(losses_ulb[0].item(), 5)})
            print('Epoch {}, iter {}, Dice Sup Loss: {}, Dice Unsup Loss: {}, BCE Sup Loss: {}, BCE UnSup Loss: {}'.format(
                epoch, iter + 1, round(losses_lb[1].item(), 5), round(losses_ulb[1].item(), 5), round(losses_lb[0].item(), 5), round(losses_ulb[0].item(), 5)
            ))
        ...
        # ----------------------------------------------------------------
        model.eval()
        model.swin_unet.decoder.set_SMem_status(epoch=epoch, isVal=True)
...
def test(config, model, model_dir, test_loader, criterion):
    ...
    model.eval()
    model.swin_unet.decoder.set_SMem_status(epoch=-1, isVal=True)
    dice_test_sum= 0
    iou_test_sum = 0
    loss_test_sum = 0
    num_test = 0
