...
from Models.Transformer.ours import SwinUnet
...
def train_val(config, model, train_loader, val_loader, criterion):
    ...
    for epoch in range(epochs):
        ...
        for idx, (batch, batch_w_s) in enumerate(source_dataset):
            ...
            lb_seg, mix_lb_seg, mix_ulb_seg, ulb_seg = model(img, weak_batch)
            lb_seg = torch.sigmoid(lb_seg)
            mix_lb_seg = torch.sigmoid(mix_lb_seg)
            mix_ulb_seg = torch.sigmoid(mix_ulb_seg)
            ulb_seg = torch.sigmoid(ulb_seg)
            
            # calculate supervised loss
            assert (lb_seg.shape == label.shape)
            lb_losses = []
            for function in criterion:
                lb_losses.append(function(lb_seg, label))
            ...
            res_losses = []
            for function in criterion:
                res_losses.append(function(mix_lb_seg, label))
                
            # Unsupervised Loss
            pseudo_mask = (mix_ulb_seg > config.semi.conf_thresh).float()
            outputs_weak_masked = mix_ulb_seg * pseudo_mask
            pseudo_outputs = torch.round(outputs_weak_masked)
            
            ulb_losses = []
            for function in criterion:
                ulb_losses.append(function(ulb_seg, pseudo_outputs))
                
            # Weak-Strong Loss
            w_s_output = model(torch.cat((weak_batch, strong_batch)))
            w_s_output = torch.sigmoid(w_s_output)
            w_output, s_output = w_s_output.split([unsup_batch_len, unsup_batch_len])
            
            pseudo_mask = (w_output > config.semi.conf_thresh).float()
            outputs_weak_masked = w_output * pseudo_mask
            pseudo_outputs = torch.round(outputs_weak_masked)
            
            w_s_losses = []
            for function in criterion:
                w_s_losses.append(function(s_output, pseudo_outputs))
            
            #======================================================================================================
            consistency_weight = get_current_consistency_weight(iter // 150)
            lb_loss = sum(lb_losses) / 2.0
            res_loss = sum(res_losses) / 2.0
            ulb_loss = sum(ulb_losses) / 2.0
            w_s_loss = sum(w_s_losses) / 2.0
            u_loss = res_loss * 0.25 + ulb_loss * 0.25 + w_s_loss * 0.5
            loss = lb_loss + u_loss * consistency_weight * (sup_batch_len / unsup_batch_len)
            
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            loss_train_sum += loss.item() * sup_batch_len
            dice_s_loss = lb_losses[1].item() * 0.25 + res_losses[1].item() * 0.25 + w_s_losses[1].item() * 0.5
            dice_u_loss = ulb_losses[1].item()
            bce_s_loss = lb_losses[0].item() * 0.25 + res_losses[0].item() * 0.25 + w_s_losses[0].item() * 0.5
            bce_u_loss = ulb_losses[0].item()
            
            # calculate metrics
            with torch.no_grad():
                output = lb_seg.cpu().numpy() > 0.5
            ...
            if config.log == True:    
                wandb.log({'Train/loss': loss.item(), 
                       'Train/IOU':iou_train,
                       'Dice_s_loss': round(dice_s_loss, 5),
                       'Dice_u_loss': round(dice_u_loss, 5),
                       'BCE_s_loss': round(bce_s_loss, 5),
                       'BCE_u_loss': round(bce_u_loss, 5)})
            print('Epoch {}, iter {}, Dice Sup Loss: {}, Dice Unsup Loss: {}, BCE Sup Loss: {}, BCE UnSup Loss: {}'.format(
                epoch, iter + 1, round(dice_s_loss, 5), round(dice_u_loss, 5), round(bce_s_loss, 5), round(bce_u_loss, 5)
            ))
        ...
        if config.log == True:
            wandb.log({'Val/loss': loss_val_epoch.item(), 'Val/Dice_score': dice_val_epoch, 'Val/IOU': iou_val_epoch})
...
if __name__=='__main__':
    ...
    exp_dir = 'checkpoints/{}_{}/fold{}'.format(args.exp, config['data']['supervised_ratio'], args.fold)
    ...
    if config.log == True: 
        wandb.login(key='3ab2a1680e5676be9934a04a9519b2802b1e4b5f')
        run = wandb.init(
            # Set the project where this run will be logged
            project="semi",
            # Track hyperparameters and run metadata
            name='{}-{}-fold{}'.format(args.exp, config['data']['supervised_ratio'], args.fold),
            config={
                "seed": args.seed,
            },
        )
